...
def validate_batch_data(table_name: str, suite_name: str):
	...
	vlog.info('validate_run_id: ', validate_run_id)
	...
	vlog.info('validate_batch_fingerprint: ', '\n', validate_batch_fingerprint)
	print('------------------------------------------------------')
	vlog.info('validate_batch_id: ', '\n', validate_batch_id)
	...
	if validation_results['success']:
		...
		vlog.warning(
			'This batch, run_id: {0} does not meet all expectations of {1:s}'.format(validate_run_id, data_asset_name))
		vlog.warning('This batch, run_id: {0} does not meet all expectations of {1:s}'.format(validate_run_id,
		                                                                                      data_asset_name))
		for test in validation_results['results']:
			if not test['success']:
				validation_rule = test['expectation_config']['expectation_type']
				column = test['expectation_config']['kwargs']['column']
				vlog.info(validation_rule, '\n', column)
