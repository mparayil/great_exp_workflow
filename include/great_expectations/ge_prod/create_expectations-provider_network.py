...
def generate_expectations(table_name: str, batch: ge.dataset.Dataset):
	...
	master_col_names = [
		...
		'DISPATCH_REQUEST_TIME_UTC', 'PROVIDER_NAME', 'PROVIDER_ADDRESS_1', 'PROVIDER_ADDRESS_2',
		'PROVIDER_ZIP', 'PROVIDER_PHONE', 'PROVIDER_FAX', 'ETA_MINUTES', 'TOTAL_ETA_EXTENSION_MINUTES',
	...
	for col, weight in weights.items():
		print(col, batch.expect_column_values_to_not_be_null(col, mostly=weight, include_config=True, catch_exceptions=True,
		                                                     result_format='BASIC'), sep='\n')

	# Expecting columns to be in an unique set at least X % of the time (X given by dict value)
	category_cols = {'PROVIDER_CATEGORY': 0.8, 'DISPATCH_RESPONSE_REASON': 0.8, 'DISPATCH_RESPONSE': 0.99, 'DISPATCH_TYPE': 0.99}
	val_set_list = []
	...
	print(
		batch.expect_column_values_to_be_between('ETA_MINUTES', min_value=0, max_value=180, mostly=0.98, result_format='BASIC',
		                                         catch_exceptions=True))
	print(batch.expect_column_min_to_be_between('ETA_MINUTES', min_value=0, catch_exceptions=True, result_format='SUMMARY'))

	print('ENROUTE VALUE RANGES')
	print(
		batch.expect_column_values_to_be_between('ENROUTE_MILES', min_value=0, max_value=100, mostly=0.98, result_format='BASIC',
		                                         catch_exceptions=True))
	...
	print(batch.expect_column_values_to_match_regex('PROVIDER_ZIP', '^(\d{5}(-\d{4})?|[A-CEGHJ-NPRSTVXY]\d[A-CEGHJ-NPRSTV-Z] ?\d[A-CEGHJ-NPRSTV-Z]\d)$'))
...
if __name__ == "__main__":
	batch_df = generate_suite_and_batch_data(table_name='provider_network', suite_name='warnings_dec2019',
